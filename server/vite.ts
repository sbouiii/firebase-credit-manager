...
export function serveStatic(app: Express) {
  // In production, the build output is in dist/public (relative to project root)
  // import.meta.dirname points to server/ directory, so we go up one level
  ...
  try {
    ...
    log(`Found ${files.length} files in dist/public: ${files.join(", ")}`);
  } catch (e) {
    log(`Warning: Could not read dist/public directory: ${e}`, "WARN");
  }
